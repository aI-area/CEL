...
class TimeSeriesTransformer(nn.Module):
    ...
    def forward(self, src):
        ...
        return output[-1]  # We're interested in the last output for the sequence
...
df = pd.read_csv('eu_measles_data.csv')
num_tasks = 6
...
epochs_to_measure = [50, 60, 70, 80, 90, 100]

# Dictionary to store training times
training_times = {}

for epochs in epochs_to_measure:
    ...
    for task, (train_data, test_data) in enumerate(all_data):
        ...
        if len(actuals) > 0 and len(predictions) > 0:
            ...
            # Store the performance for each task
            stored_performances.append((r2, rmse))
        else:
            print(f"Insufficient data for evaluation in task {task + 1}")
    end_time = time.time()
    training_time = end_time - start_time
    training_times[epochs] = training_time
    print(f"Training for {epochs} epochs completed in {training_time:.2f} seconds")

# Re-evaluate the model on all tasks
for task, (train_data, test_data) in enumerate(all_data):
    # Reset predictions and actuals for each task
    predictions = []
    actuals = []
